...
- Breaking: Canonicalize every numeric semantic `.H` value to exactly four uppercase hexadecimal digits (`0000` through `FFFF`). Structural timer/counter status remains the exact PLC token `0` or `1`; raw response bytes and hexadecimal write spelling are unchanged.
...
- Docs: Document the request-identifier-free TCP residual race and the decision to retain persistent healthy connections instead of adding a handshake to every request.
- Tests: Verify that monitor registration is connection-scoped and an explicit reopen requires registration again before monitor reads.
- Library: Preserve ordered `MWS` formats and validate every `MWR` token positionally as `.U`, `.S`, `.H`, `.D`, `.L`, or a bare-wire packed direct-bit unsigned 16-bit field before returning any values.
- Library: Reuse one connected UDP socket and local endpoint after complete valid exchanges. Timeout, cancellation, transport/protocol or malformed-response failure, extra responses, and pre-send unowned datagrams discard only that socket; the next operation creates a replacement from the resolved endpoint without repeating DNS.
- Library: Keep the semantic TCP or logical UDP connection open after any complete PLC `E0` through `E9` response, including state-changing commands; malformed and transport failures retain their existing retirement and outcome-unknown behavior.
- Library: Treat a descending named-read address as a segment boundary instead of abandoning the compiled plan; the new segment can grow across subsequent contiguous ascending addresses while results retain input order.
- Tests: Add cross-library reaudit regressions for TCP surplus responses, format-aware monitor reads, canonical hexadecimal projections, `Z:F` preflight rejection, all PLC error codes, UDP socket generations and delayed/foreign datagrams, close races, and descending named-read plans.
...
- Breaking: Float32 parsing, formatting, typed access, named reads, and polling now accept only canonical ordinary `.U` word families (`DM`, `EM`, `FM`, `ZF`, `W`, `TM`, `CM`, `VM`, `D`, `E`, `F`); `Z`, direct-bit, and special-response families such as `R`, `T`, `C`, and `AT` fail before FIFO admission and transport.
...
- Library: Dropping a waiting future sends nothing. Dropping an active future returns no library result, immediately drops the owned request socket, and requires callers to treat a possibly transmitted state-changing operation as unknown. TCP needs explicit reopen; UDP replaces the socket from its resolved logical endpoint on the next command without retrying the abandoned request.
- Library: Make `read_named` and each poll cycle one all-or-error FIFO turn. Prevalidate the entire address set, group wire-compatible device types by first appearance, sort addresses within each group, merge ranges to the minimum request count, keep multiword values whole at segment boundaries, and restore public results to input order. Multi-frame reads remain non-PLC-atomic.
- Library: Release the named-read FIFO turn immediately after final decode and all-or-error staging, before pure `NamedReadResult` materialization; poll intervals remain outside the turn.
