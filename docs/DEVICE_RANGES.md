...
- `supported = false` means the design table entry is `-`.
...
catalog. They are split into standard and `XYM` catalog profiles to keep them
readable in GitHub-style Markdown renderers.
...
- XYM profiles may remap the same logical row to alias devices such as `X`, `Y`,
  `D`, `E`, `F`, `M`, and `L`.
